...
useEffect(() => {
  ...
  // Fetch projects and find the one matching the slug
  axios
    .get(apiUrl)
    .then((response) => {
      ...
      if (response.data.success) {
        ...
        console.log('Projects fetched:', apiProjects);

        const foundProject = apiProjects.find(
          (p) => (p.slug || slugify(p.name || 'untitled-project', { lower: true, strict: true })) === slug
        );
        ...
        if (foundProject) {
          setProject({
            ...
            price: foundProject.tag_price ? `₹ ${foundProject.tag_price} CR* ONWARDS` : 'Price on Request',
            location: foundProject.address,
            size: foundProject.specification,
            feet: '1948 - 3700 Sq.Ft.', // Adjust based on API data if available
            image: foundProject.hero_img,
            disclaimer: foundProject.disclaimer,
            amenities: foundProject.amenities,
            tag_line: foundProject.tag_line,
  ...
  // Optionally, check static projects if API doesn't have the project
  ...
  // console.log('Static project check:', staticProject);
  // if (staticProject) {
  //   setProject(staticProject);
  //   console.log('Static project set:', staticProject);
  //   setLoading(false);
  // } else {
  //   console.log('No static project found for slug:', slug);
  // }
}, [slug]);
